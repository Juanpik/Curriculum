...
/* ─── SCROLL REVEAL ─────────────────────────────────── */
// Only activate animations if IntersectionObserver is supported
if ('IntersectionObserver' in window) {
  // Signal CSS that JS is ready — enables the hidden state
...
function setActiveNavLink() {
  ...
  sections.forEach(section => {
    ...
    if (link) {
      link.style.color = (scrollY >= top && scrollY < bottom) ? 'var(--accent)' : '';
    }
  });
}
...
if (contactForm) {
  contactForm.addEventListener('submit', function(e) {
    ...
    const bodyTxt = encodeURIComponent('Hola Juan Pablo,\n\nMi nombre es ' + nombre + '.\n\n' + mensaje + '\n\n---\nCorreo: ' + email);
    window.location.href = 'mailto:juanpik@gmail.com?subject=' + subject + '&body=' + bodyTxt;
...
function showFormMessage(msg, type) {
  ...
  el.textContent = msg;
  el.style.cssText = 'text-align:center;font-size:0.85rem;padding:0.7rem 1rem;border-radius:6px;margin-top:1rem;' +
...
function typeEffect() {
  ...
  glyphLang.textContent = deleting
...
if ('IntersectionObserver' in window) {
  ...
  const counterObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          entry.target.dataset.target = parseInt(entry.target.textContent.replace('+', ''), 10);
...
/* ─── CONSOLE SIGNATURE ─────────────────────────────── */
console.log('%c JPC Portfolio 2026 ', 'background:#C4622D;color:#fff;font-size:14px;padding:4px 10px;border-radius:4px;font-family:monospace;');
